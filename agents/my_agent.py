...
# Lazy import inside methods to avoid circular imports
def _save_agent_to_db(agent_obj):
    from app import db_utils as _db_utils  # local import breaks circular dependency
    _db_utils.save_agent(agent_obj)


def _delete_agent_from_db(agent_id):
...
class MyAgent:
    def __init__(self, id=None, role=None, backstory=None, goal=None, temperature=None, allow_delegation=False, verbose=False, cache=None, llm_provider_model=None, max_iter=None, created_at=None, tools=None, knowledge_source_ids=None, memory_files=None, schedule_triggers=None, **kwargs):
        ...
        # Optional advanced parameters (ignored by older UI but kept for persistence)
        self.memory_files = memory_files or []
        self.schedule_triggers = schedule_triggers or []

        # Store any other unexpected keyword arguments so loading doesn’t crash on future schema changes
            ...
            with st.expander(f"Agent: {self.role}", expanded=True):
                with st.form(key=form_key):
                    ...
                    if 'knowledge_sources' in ss and len(ss.knowledge_sources) > 0:
                        ...
                        valid_knowledge_sources = [ks_id for ks_id in self.knowledge_source_ids if ks_id in knowledge_source_options]
