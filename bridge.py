...
from .types import BiometricEvent, BiometricReading, NEUROSState, CognitiveMode, WearableType
...
class BiometricKafkaLangGraphBridge:
    ...
    async def _analyze_biometrics(self, event: BiometricEvent, 
        ...
        # Extract biometric readings from event
        hrv_reading = next((r for r in event.readings if r.metric == "hrv"), None)
        hr_reading = next((r for r in event.readings if r.metric == "heart_rate"), None)
        stress_reading = next((r for r in event.readings if r.metric == "stress_level"), None)
        recovery_reading = next((r for r in event.readings if r.metric == "recovery_score"), None)
        
        # Analyze HRV if available
        if hrv_reading:
            hrv_baseline = baseline.get("hrv", 60)
            hrv_drop = max(0, hrv_baseline - hrv_reading.value)
            hrv_drop_percentage = (hrv_drop / hrv_baseline) * 100 if hrv_baseline > 0 else 0
            
            analysis["metrics"]["hrv"] = {
                "current": hrv_reading.value,
        ...
        if hr_reading:
            ...
            hr_elevation = max(0, hr_reading.value - hr_baseline)
            
            analysis["metrics"]["heart_rate"] = {
                "current": hr_reading.value,
                "baseline": hr_baseline,
                "elevation": hr_elevation,
                "elevated": hr_reading.value > 100,
        ...
        if stress_reading:
            analysis["metrics"]["stress"] = {
                "level": stress_reading.value,
                "high": stress_reading.value > 0.7,
                "critical": stress_reading.value > 0.85
            }
        else:
        ...
        if recovery_reading:
            analysis["metrics"]["recovery"] = {
                "current": recovery_reading.value
            }
    ...
    async def _trigger_mode_switch(self, user_id: str, session_id: str,
        ...
        try:
            ...
            state_update = {
                ...
                "hrv_current": next((r.value for r in biometric_event.readings if r.metric == "hrv"), None),
                "heart_rate": next((r.value for r in biometric_event.readings if r.metric == "heart_rate"), None),
                "stress_level": next((r.value for r in biometric_event.readings if r.metric == "stress_level"), None),
                ...
                "mode_history": current_state.get("mode_history", []) + [{
                    ...
                    "biometric_snapshot": {
                        "hrv": next((r.value for r in biometric_event.readings if r.metric == "hrv"), None),
                        "heart_rate": next((r.value for r in biometric_event.readings if r.metric == "heart_rate"), None),
                        "stress_level": next((r.value for r in biometric_event.readings if r.metric == "stress_level"), None)
                    }
                }]
    ...
    async def _update_redis_biometrics(self, user_id: str, 
        ...
        try:
            with redis_operation_duration.labels(operation="update_biometrics").time():
                ...
                hrv_value = next((r.value for r in biometric_event.readings if r.metric == "hrv"), 0)
                hr_value = next((r.value for r in biometric_event.readings if r.metric == "heart_rate"), 0)
                stress_value = next((r.value for r in biometric_event.readings if r.metric == "stress_level"), 0)
