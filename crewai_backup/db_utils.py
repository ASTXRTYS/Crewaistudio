...
# Import tool registry with fallback logic that works both when `db_utils` is
# imported as a top-level module (e.g. "import db_utils") *and* when it’s
# imported via the package path "app.db_utils".
try:
    # When running from app directory, use relative import
    from .my_tools import TOOL_CLASSES as _TOOL_CLASSES  # type: ignore  
except (ImportError, AttributeError):
    # Fallback to absolute import from root
    try:
        from my_tools import TOOL_CLASSES as _TOOL_CLASSES  # type: ignore
    except (ImportError, AttributeError):
