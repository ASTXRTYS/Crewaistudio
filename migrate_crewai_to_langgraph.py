...
class CrewAIToLangGraphMigrator:
    ...
    def __init__(self):
        ...
        self.backup_dir = Path("langgraph_backup")
    ...
    def migrate_imports(self, content: str) -> str:
        ...
        replacements = [
            # Core imports
            (r'from typing import TypedDict, Annotated, List
from langgraph.graph import StateGraph, START, END\b', 'from typing import TypedDict, Annotated, List\nfrom langgraph.graph import StateGraph, START, END'),
            (r'from langchain_core.messages import BaseMessage\b', 'from langchain_core.messages import BaseMessage'),
            (r'from langgraph.graph import StateGraph\b', 'from langgraph.graph import StateGraph'),
            (r'from langgraph.graph.message import add_messages\b', 'from langgraph.graph.message import add_messages'),
            (r'from langchain_openai import ChatOpenAI\b', 'from langchain_openai import ChatOpenAI'),
            ...
            # Other imports
            (r'from typing import Dict, Any\b', 'from typing import Dict, Any'),
            (r'import langgraph\b', 'import langgraph'),
        ]
...
{class_name.lower()}_tool = Tool(
    ...
    def migrate_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            # Additional cleanup
            content = re.sub(r'\.get_langgraph_\w+\(\)', '', content)
            content = re.sub(r'langgraph_', 'langgraph_', content)
